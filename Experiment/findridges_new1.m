...
for j = (min(tmax+1,N)):N
    ...
    low = max(1,f0-delta);
    up = min(M,f0+delta);
    %[~,f0] = max(Spec(low:up,j)+1./(1+abs(Specangle(low:up,j)-theta)));
        [~,f0] = max(Spec(low:up,j)-abs((Specangle(low:up,j)-theta))/180);
...
for j = (max(1,tmax-1)):-1:1
    ...
    low = max(1,f1-delta);
    up = min(M,f1+delta);
    %[~,f1] =  max(Spec(low:up,j)+1./(1+abs(Specangle(low:up,j)-theta)));
     [~,f1]= max(Spec(low:up,j)-abs((Specangle(low:up,j)-theta))/180);
