...
% addpath('D:\tfsa_5-5\windows\winWN_bin');

WN=64; N=128;
n=0:N-1+WN*0;

Sig1 = exp(1i*(2*pi*(0.1*n.^3)/(N*N))+1i*(2*pi*(0.05*n))+1i*(2*pi*(0.05*n.^2/N))); %300t»???150t
Sig2= exp(1i*(2*pi*(0.1*n.^3)/(N*N))+1i*(2*pi*(0.15*n))+1i*(2*pi*(0.05*n.^2/N)));
...
%  S_L=56;
  ...
  S_L=64; % Number of missing samples
  p = randperm(N+WN*0); % In order to have a fair comparison, I have ensured that samples are missed in the same interval.
% load Data;
  % S_L=32;
...
for tt=0:WN/wind_step:length(x_p)-WN-1    % Signal length (1024+256) with a step of 32
  ...
  [extr_Sig2]=extridge_mult_tffiltering_multiple_tfd_viterbi(x_s(WN+1:N+WN), 2, 5,9,x_p(WN+1:N+WN),p(1:S_L),5);
  
  figure;
  [tfd3,orienttfd]=HTFD_new1(extr_Sig2,2,20,84); tfsapl(x_o(1:N),tfd3);
...
%  Sig_r=Sig_r(1:N);
 figure;
 [tfd3,orienttfd]=HTFD_new1(Sig_r(WN+1:N+WN),2,20,84);
tfsapl(x_o(1:N),tfd3); title('Stankovic Method');
...
display('Stankovic Method')
 mean(abs(x_o(1:N)-Sig_r(WN+1:N+WN)))
 display('The Proposed')
 mean(abs(x_o(1:N)-extr_Sig2(1:N)))

% I=quadtfd(Sig_r(1:N),N-1,1,'specx',31,'hamm',N);
% figure
% tfsapl(Sig_r,abs(I).^2);

% Error in dB in time domain :
 %Error_DB_time1=10*log10(mean(abs(x_o(WN:N)-Sig_r(WN:N)).^2))
 %Error_DB_time2=10*log10(mean(abs(extr_Sig2(WN:N)-Sig_r(WN:N)).^2))
