function [extr_Sig1] = extridge_mult_tffiltering_multiple_tfd_viterbi(Sig, num, delta,bw,Seg,iii,iter)
WIN=length(Sig)/2;
if (isreal(Sig))
    Sig = hilbert(Sig);
end
...
for ii=1:iter
    ...
    for i=1:num
        ...
        s3(WIN-L:WIN+L)=s2(WIN-L:WIN+L).*blackman(2*L+1)';
        s2(WIN-L:WIN+L)=0;
